...
export const options = {
  thresholds: {
    ...
    http_req_duration: [{ threshold: 'p(99)<30000', abortOnFail: false }], // Raised threshold and disabled abort
  },
  ...
  scenarios: {
    stress_ramp: {
      ...
      stages: [
        ...
        { duration: '20s', target: 300 },  // Light load
        { duration: '20s', target: 500 },  // Moderate
        { duration: '20s', target: 800 },  // Heavy
        { duration: '20s', target: 1000 }, // Stress Max VUs
...
export function setup() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const uniqueName = `StressUser_${i}_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    const res = http.post(`${BASE_URL}/auth/register`,
      JSON.stringify({ email, password: 'BenchPass@123', name: uniqueName }),
      { headers: jsonHeaders() }
    );
